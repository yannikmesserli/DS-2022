require([
    ...
    const scene = new WebScene({
      portalItem: {
          id: "2ba10b179dec45e3862ee41fec090a1b"
      }
    });
    ...
    async function enableStats() {
      ...
      const layerView = await view.whenLayerView(earthquakesLayer);
      await watchUtils.whenNotOnce(layerView, "updating");

      view.on("pointer-move", function (event) {
        event.stopPropagation();
        queryStats(layerView, event);
      });
    }
    ...
    const queryStats = promiseUtils.debounce(function (
      layerView,
      event
    ) {
      if (view.toMap(event)) {
